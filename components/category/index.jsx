        ...
        <div id="content">
          <ul className="mx-auto" id="headline">
            {headline.map(headline => (
              <LargeImage key={headline.slug} article={headline} 
                          withSummary={true} isHeadline={true} border={true} />
            ))}
          </ul>
          <div className="row" id="mid-content">
            {category.map(article => (
              <div className="col-12 col-md-4" id="no-sum-large" key={article.slug}>
                <ul>
                  <LargeImage article={article} withSummary={false} border={true} />
                </ul>
              </div>
            ))}
            {category.map(article => (
